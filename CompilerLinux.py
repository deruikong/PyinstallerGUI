...
class MainFrame(wx.Frame):
    ...
    def inpath(self):
        if not 'usr/bin' in os.environ['PATH']:
            warn = wx.MessageDialog(None,'Seems you didn\'t Add Python to PATH. Please add it then restart', 'Warning',wx.OK|wx.ICON_WARNING)
            warn.ShowModal()
            sys.exit()
    ...
    def run(self,event):
        ...
        if '.py' in filename or '.spec' in filename:
            if '.ico' in self.iconentry.GetValue():
                control = 'python2 -m PyInstaller %s %s -i="%s" %s' % (filetype, guicli, self.iconentry.GetValue(), filename)

            else:
                control = 'python2 -m PyInstaller %s %s %s' % (filetype, guicli, filename)
            os.system(control)
            ...
            if realname in str(os.listdir(os.getcwd() + '/dist')):
